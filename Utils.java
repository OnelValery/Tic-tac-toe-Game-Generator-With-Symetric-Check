...
public class Utils extends TicTacToeGame {
    ...
    public static void rotate(int lines, int columns, int[] transformedBoard){
      ...
      if(lines==columns) {
    	int[] flip;
	   flip = new int [lines*columns];
		for(int i=0;i<lines;i++) {
			for (int j=0;j<columns;j++) {	
				int size;
				size =((lines-1)-j)*columns +i;
				flip[columns*i+j]= transformedBoard[size];	
			}
		
		}
		for (int i=0;i<lines*columns;i++) {
			transformedBoard[i]=flip[i];}
    }
    ...
    public static  void horizontalFlip(int lines, int columns, int[] transformedBoard){
    	...
    	rotate(lines,columns, transformedBoard);
    	rotate(lines,columns, transformedBoard);
 	   ...
 	   flip = new int [lines*columns];
 		...
 		for(int i=lines-1;i>=0;i--) {
 			for (int j=columns-1;j>=0;j--) {	
 				int size;
 				size =((lines-1)-i)*columns +j;
 				flip[(lines-(i+1))*columns +(columns-(j+1))]= transformedBoard[size];	
 			}
 		}
