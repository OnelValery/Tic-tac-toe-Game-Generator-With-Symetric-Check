...
public class TicTacToeGame {
	...
	*/
	...
	int transIndex;
	private Transformation[] transformat;
    ...
    *  the number of cells that must be aligned to win.
	...
	public TicTacToeGame(int lines, int columns, int sizeWin){
		...
		for (int k=0;k< lines*columns;k++) {
	  	transformedBoard[k]=k;
	  	}
		
		// UPDATE HERE IF NEEDED
		//transIndex=0;
		if (this.lines==this.columns) {
			transformat= new Transformation[8];
			transformat[0]= Transformation.ID;
			transformat[1]= Transformation.ROT;
			transformat[2]= Transformation.ROT;
			transformat[3]= Transformation.ROT;
			transformat[4]= Transformation.HSYM;
			transformat[5]= Transformation.ROT;
			transformat[6]= Transformation.ROT;
			transformat[7]= Transformation.ROT;
		}
		else if (this.lines!=this.columns) {
			transformat= new Transformation[4];
			transformat[0]= Transformation.ID;
			transformat[1]= Transformation.HSYM;
			transformat[2]= Transformation.VSYM;
			transformat[3]= Transformation.HSYM;
    ...
    *  String representation of the game
    ...
    public boolean hasNext(){
    	...
    	// YOUR CODE HERE
    	return (transIndex<transformat.length);
    ...
    public void next(){
    	 ...
    	 if (this.hasNext()) {
    		if (transformat[transIndex]==Transformation.ID) {
    			for (int k=0;k< lines*columns;k++) {
    			  	transformedBoard[k]=k;}
    		}
    		else if (transformat[transIndex]==Transformation.ROT) {
            	Utils.rotate(lines, columns, this.transformedBoard);	
            }
    		else if (transformat[transIndex]==Transformation.HSYM) {
            	Utils.horizontalFlip(lines, columns, transformedBoard);	
            }
    		else if (transformat[transIndex]==Transformation.VSYM) {
            	Utils.verticalFlip(lines, columns, transformedBoard);	
            }
    			transIndex++;
    			transform = new CellValue[this.lines*this.columns];

    			for (int i=0;i<this.lines*this.columns;i++) {
        			transform[i]=this.board[transformedBoard[i]];
        			}
    	}
    ...
    *  the TicTacToeGame instance to be compared with this one
  	*/    
  	public boolean equalsWithSymmetry(TicTacToeGame other){
  		...
  		toReturn = new CellValue[this.lines*this.columns];
  		for(int k=0;k<this.lines*this.columns;k++) {
  			if(this.board[k]==CellValue.EMPTY) { toReturn[k]=CellValue.EMPTY;}
  			else if(this.board[k]==CellValue.X) { toReturn[k]=CellValue.X;}
  			else { toReturn[k]=CellValue.O;}
  		}
     ...
     /**
	* Returns a String representation of the game as currently transformed
	* 
   	* @return
    *  String representation of the game
  	*/
